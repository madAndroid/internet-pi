...
case "$1" in
	...
	ALL)
		...
		;;
	*)
		echo "Usage: $(basename "$0") {k8s|nas|ALL}"
		exit 1
		;;
esac
...
if [ "$DO_K8S" = true ]; then
	...
	### Makes this script safe to re-run after a prior attempt already got as
	### far as shutting down nodes (e.g. killed by Home Assistant's 60s
	### timeout partway through) - the API being unreachable means there's
	### nothing left to gracefully cordon/scale down, so skip straight to
	### telling the (possibly still-up) nodes to shut down. Bounded with
	### --request-timeout, since kubectl's own default wait against a dead
	### apiserver is much longer than we can afford here.
	...
	if kubectl get --raw='/healthz' --request-timeout=5s >/dev/null 2>&1; then
		...
		K8S_API_UP=false
		echo "Kubernetes API is not reachable - skipping cordon/scale-down"
	fi

	if [ "$K8S_API_UP" = true ]; then
